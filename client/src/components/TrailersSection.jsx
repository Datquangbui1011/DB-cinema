...
const TrailersSection = () => {
    ...
    const getYouTubeId = (url) => {
        ...
        return match ? match[1] : url; // If no match, assume it's already an ID
    };

    // Filter and map movies from shows context
    const showingMoviesWithTrailers = shows
        .filter(movie => movie.trailer_path)
        .map(movie => ({
            videoUrl: `https://www.youtube.com/watch?v=${movie.trailer_path}`,
            image: `${image_base_url}${movie.backdrop_path}`,
            title: movie.title
        }))
        .slice(0, 4); // Keep it to 4 trailers as per original design

    useEffect(() => {
        if (showingMoviesWithTrailers.length > 0 && !currentTrailer) {
            setCurrentTrailer(showingMoviesWithTrailers[0]);
        }
    }, [showingMoviesWithTrailers, currentTrailer]);
    
    const handleTrailerChange = (trailer) => {
        setCurrentTrailer(trailer);
    };

    if (showingMoviesWithTrailers.length === 0) {
        return null; // Or a skeleton/empty state
    }
    ...
    return (
        <div className='px-6 md:px-16 lg:px-24 xl:px-44 py-20 overflow-hidden '>
            ...
            <div className='group grid grid-cols-4 gap-4 md:gap-8 mt-12 max-w-3xl mx-auto'>
                {showingMoviesWithTrailers.map((trailer, index) => (
                    <div 
